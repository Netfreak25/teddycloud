...
The same packet writer also accepts the independent Ogg/Opus chapters of a
native TB2 library collection. Native chapters start at byte zero; existing TAF
playlist entries still start after the 4096-byte TAF header. All existing TAP
callers explicitly retain the TAF mode, so their generation semantics do not
change.
...
- What changed: extends the TAP model with `shuffle`, a backend-only random-candidate limit, runtime index helpers, live-header prediction, replace-safe publish APIs and synchronous final-snapshot materialization for V3.
...
- What changed: owns TAP live streaming, per-target locking, prediction-based local `Content-Length`, generator lifecycle, handler-owned publish, freshness decisions, V3 content-meta protection and pro-active V3 freshness invalidation. The freshness comparison is shared by normal checks and content-mapping updates. V3 content-meta and freshness use the same effective server version and the same `nocloud`/`cloud_override` policy, including overlay-local replacement versions after source changes. For V3, a live TAP is synchronously materialized under the target lock before immutable Ogg/Opus chapters and the generation descriptor are created. Source-change invalidations remain pending through content-meta and chapter requests and are cleared only by matching MQTT playback state. A mapping change queues the exact affected UID for MQTT freshness delivery.
- Why: the HTTP handler is the only component that owns target serialization, source selection, the effective overlay version and local-versus-TONIES routing. V3 requires an immutable complete generation before content-meta becomes visible; a growing `.taf.tmp` cannot provide that contract.
- Why not another solution: using `.tmp` directly can publish incomplete or changing chapter bytes; letting Meta and Chapter choose shuffle order independently can describe and serve different content; clearing freshness on an HTTP response does not prove that the box activated the new generation.

### `src/mqtt_server.c`

- What changed: `fresh-tonies` publishing now sends one `{"tonie":"<RUID>"}` object per deduplicated cache UID, sequentially with MQTT QoS 1. It waits for `PUBACK`, retries twice at five-second intervals with the same packet ID and `DUP=1`, and closes the connection after the third attempt times out. The queue works for local and transparent-proxy sessions, observes proxy subscriptions, requeues pending cache entries after reconnect, and no longer pauses for active playback.
- Why: the TB2 protocol expects one Tonie per message and reliable ordered delivery. MQTT acknowledgement and activation are separate: `PUBACK` advances the connection queue, while a source-change entry remains pending until matching playback reports the effective content version.
...
- What changed: implements TAP load validation, shuffle/runtime indices, TAF-only remux, live-header prediction, replace-safe publish, generator task lifecycle and synchronous final-snapshot materialization. The snapshot API selects runtime indices once, generates a complete TAF, publishes it to the final path and returns only that final path. Normal and shuffle-all playlists are limited by the runtime TAF source/chapter limit; shuffle-one playlists may load a larger candidate catalog because only one entry is selected for generation. The FFmpeg fallback passes the TAP `audio_id` into generated finals when the playlist has a stable non-zero version.
